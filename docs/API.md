...
```json
{ "total": 4, "imported": 4, "updated": 0, "errors": [], "warnings": [],
...
- 幂等键：**职位名+隶属公司+国家或地区+开启日**（4 列，PRD §3.1 v2.3）；库内已存在则更新字段（保留编号/事件/员工关联）；文件内幂等键重复 → 报错该行不导入。Web 导入默认 strict 模式：法律分类不在主数据字典的行报错不导入（CLI 可 `--strict-legal` 关闭为告警）。
