...
- **成本模式互斥服务端兜底（issue #141/#158 无条件重算覆写）**：最终模式为 auto 时，凡本次请求触及任一成本输入（薪资/奖金/派生栏）或发生 manual→auto 翻转，服务端即按公司绑定税区引擎**重算并覆写**强制扣税/定额扣费/用工成本三栏——手填残留值一律以引擎结果为准，使「auto ⇒ 派生 == 引擎」成为无条件不变式；公司未绑税区 / 税区无有效科目 / 薪资缺失时三栏置 null。奖金两栏与薪资为输入项不受影响。另：成本六栏支持显式 `null` 清空（model_fields_set 口径）。员工 `actual_cost_mode` 同构（虚拟建档 zone=None → 派生栏置 null）。
...
- **首次挂编**：外包虚拟建档员工（无在挂岗位）可经本接口挂编到首个岗位（issue #140）；
  目标岗位状态白名单为 **Open/Vacant/Offered**（不含 Planned——普通挂编守 F1.5 条件，
  与 claim/promote 的 F1.5b 特例白名单刻意不对称）；目标岗位公司的写权限按可管实体校验（issue #147）。
...
- 幂等键：**职位名+隶属公司+国家或地区+开启日**（4 列，PRD §3.1 v2.3）；库内已存在则更新字段（保留编号/事件/员工关联）；文件内幂等键重复 → 报错该行不导入。strict 模式为全局默认（#51，#148 扩展）：**法律分类与职位类型**两类不在规范字典/枚举（法律四类 / Consultant·Employee·External Employee 三类）的行报错不导入；仅历史数据迁移可显式关闭——CLI `--no-strict-legal` 同时放宽两类。
